...
export default class PhoneList extends Component {
    ...
    render() {
        ...
        const renderItem = ({item}) => {
            ...
            return (
                item.id ?
                    <TouchableOpacity style={[styles.listItem,PublicStyles.ju_start_Direction,PublicStyles.al_center]} onPress={onPress}>
                        <Image style={PublicStyles.ChatPic} source={require('../../img/userPic.jpg')} />
                        <Text style={styles.itemName}>{`${item.name}`}</Text>
                        <Text style={PublicStyles.ft16_333}>15110710511（托一班）</Text>
                    </TouchableOpacity> :
